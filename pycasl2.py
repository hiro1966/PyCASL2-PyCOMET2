...
##             addr += len(c.code)
        ...
        list(labels).sort(key=lambda x:x.lines)
        for i in labels:
            print(i)
...
##             print >> sys.stderr, i
        ...
        def conv(x, bcode):
            if type(x) == type('str'):
                ...
                # スコープ内にないときは、スコープ名なしのラベルを探す
                    ...
                    if self.symbols[global_name].goto == '':
    ...
    def split_line(self, line, line_number):
        ...
        re_args = r"(\s+%s(\s*,\s*%s(\s*,\s*%s)?)?)?" % (re_arg1, re_arg2, re_arg3)
        re_comment = r"(\s*(;(?P<comment>.+)?)?)?"
