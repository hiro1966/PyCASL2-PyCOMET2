...
class pyComet2:
    ...
    class StatusMonitor:
        ...
        def append(self, s):
            ...
            try:
                ...
                print ("Warning: Invalid monitor target is found. %s is ignored." % s,file=sys.stderr)
        ...
        while (True):
            if self.PR in self.break_points:
                ...
                try:
                    ...
                    sys.stdout.flush()
                    self.step()
                except self.InvalidOperation as e:
    ...
    def disassemble(self, start_addr = 0x0000):
        ...
        for i in range(0, 16):
            try:
                ...
                if inst != None:
                    print ('#%04x\t#%04x\t%s' % (addr, self.memory[addr], inst.disassemble(addr)),file=sys.stderr)
                    if 1 < inst_size[inst.argtype]:
                        print ('#%04x\t#%04x' % (addr+1, self.memory[addr+1]),file=sys.stderr)
                    if 2 < inst_size[inst.argtype]:
                        print ('#%04x\t#%04x' % (addr+2, self.memory[addr+2]),file=sys.stderr)
                    addr += inst_size[inst.argtype]
                else:
                    print ( '#%04x\t#%04x\t%s' % (addr, self.memory[addr], '%-8s#%04x' % ('DC', self.memory[addr])),file=sys.stderr) 
                    addr += 1
            except:
                print ('#%04x\t#%04x\t%s' % (addr, self.memory[addr], '%-8s#%04x' % ('DC', self.memory[addr])),file=sys.stderr)
                addr += 1
    ...
    def print_help(self):
        print ('b ADDR        Set a breakpoint at specified address.',file= sys.stderr)
        print ('d NUM         Delete breakpoints.',file= sys.stderr)
        print ('di ADDR       Disassemble 32 words from specified address.',file= sys.stderr)
        print ('du ADDR       Dump 128 words of memory.',file= sys.stderr)
        print ('h             Print help.',file= sys.stderr)
        print ('i             Print breakpoints.',file= sys.stderr)
        print ('j ADDR        Set PR to ADDR.',file= sys.stderr)
        print ('m ADDR VAL    Change the memory at ADDR to VAL.',file= sys.stderr)
        print ('p             Print register status.',file= sys.stderr)
        print ('q             Quit.',file= sys.stderr)
        print ('r             Strat execution of program.',file= sys.stderr)
        print ('s             Step execution.',file= sys.stderr)
        print ('st            Dump 128 words of stack image.',file= sys.stderr)
